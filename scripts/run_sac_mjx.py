#!/usr/bin/env python3
"""
SAC for MuJoCo Playground DMC Suite envs.
Uses brax.training.agents.sac.train — fully XLA, GPU replay buffer, zero Python per step.

Architecture (official Brax SAC):
  - GPU replay buffer: brax.training.replay_buffers.UniformSamplingQueue
  - Full epoch in jax.lax.scan (num_training_steps_per_epoch steps per JIT call)
  - jax.pmap over local devices
  - ~4000-8000 sps on RTX 3090
