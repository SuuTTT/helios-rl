...
v22 = v20 (K_UPDATE=256) + target network fixed during gradient scan.

Root cause of v20 plateau at MPPI~234:
  With TAU=0.01 per gradient step and K_UPDATE=256 scanned steps per env batch:
  target = (1-0.01)^256 * old = 7.7% old + 92.3% online after each env batch.
  Target tracks online almost instantly → bootstrapping target = online Q → instability.

Fix: Move target update OUTSIDE the K_UPDATE scan.
  During all 256 gradient steps: target is FIXED (good reference for TD targets).
  After all 256 steps: single EMA update with TAU=0.01 (matches official per-env-step rate).
  This is the standard DQN/TD-MPC2 approach: fixed target for multiple gradient steps.

Key changes vs v20:
  1. one_step() in multi_step: tp removed from carry, closed over (fixed during scan)
  2. After scan in multi_step: single target EMA update with TAU=0.01
  3. K_UPDATE=256 preserved (4x more gradient steps per env step)
  4. All other settings unchanged
"""
import os, sys, time
import numpy as np
...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx,
    ...
    def multi_step(params, tp, opt, all_obs, all_acts, all_rews, all_done, key):
        ...
        v22 fix: target `tp` is FIXED during the K_UPDATE scan steps.
        After all gradient steps, target is updated ONCE with TAU=0.01.
        This matches official TD-MPC2's per-env-step target EMA.
        """
        def one_step(carry, batch):
            params, opt, key = carry  # tp is FIXED (closed over from outer scope)
            ob, ab, rb, db = batch
            ...
            # No target update inside scan — target stays fixed
            return (new_params, nopt, key), (loss, aux)

        batches = (all_obs, all_acts, all_rews, all_done)
        (params, opt, key), (losses, auxs) = jax.lax.scan(
            one_step, (params, opt, key), batches)
        # Single target EMA update after all K_UPDATE gradient steps
        new_tp = jax.tree_util.tree_map(
            lambda t, p: (1 - tau)*t + tau*p, tp, params)
        last_aux = jax.tree_util.tree_map(lambda x: x[-1], auxs)
        return params, new_tp, opt, key, losses[-1], last_aux
...
def main():
    ...
    H               = 3           # Phase 1: was 5
    SEQ             = H + 1       # T = H + 1
    K_UPDATE        = 256          # v20: match official 1:1 gradient:env step ratio
    LR              = 3e-4
    LATENT          = 128
    HIDDEN          = (128, 128)
    GAMMA           = 0.99
    ...
    csv = f"{out_dir}/hopper-hop-v22.csv"
    ...
    print("Training v22: K_UPDATE=256 + fixed target during scan...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
