...
v20 = v13 + K_UPDATE=256 (matching official 1:1 gradient:env step ratio).

Key insight: Official TD-MPC2 does 1 gradient step per env step (1:1 ratio).
Our v13 does K_UPDATE=64 per N_ENVS=256 env steps = 0.25:1 ratio.
4x fewer gradient updates per data point → slower sample efficiency.
v13 achieves MPPI=388@4M vs official=594@4M. Target: close this 1.5x gap.

Changes vs v13:
  1. K_UPDATE=64 → 256 (1 gradient step per env step, matching official)
  2. All other settings unchanged (latent=128, no RunningScale, H=3, NS=512)

Expected: 4x more gradient efficiency → better convergence at 1M and 4M.
Risk: Overfitting to 32k buffer if gradient steps >> buffer diversity.
"""
import os, sys, time
import numpy as np
...
def main():
    ...
    csv = f"{out_dir}/hopper-hop-v20.csv"
    ...
    print("Training v20: K_UPDATE=256 (1:1 gradient:env step ratio)...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
