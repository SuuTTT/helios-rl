...
def main():
    run_name = f"cartpole_jax_fast_logged_{int(time.time())}"
    ...
    train_fn, config = make_train()
    jitted_train = jax.jit(train_fn)
    
    print(f"Tracking run at {log_dir}...")
    start_time = time.time()
    
    # 2. XLA executes instantaneously!
    runner_state, metrics = jitted_train(rng)
    
    run_time = time.time() - start_time
    sps = int(config["TOTAL_TIMESTEPS"] / run_time)
    print(f"\nCompleted compiling and running inside XLA in {run_time:.2f}s. SPS: {sps}")
    
    # 3. Post-process metric tensors instantly!
    start_post = time.time()
    # metrics are stacked [NUM_UPDATES, ...]
    returned_episode_returns = np.array(metrics["returned_episode_returns"])
    returned_episode = np.array(metrics["returned_episode"])
    loss_total = np.array(metrics["loss_total"])
    loss_actor = np.array(metrics["loss_actor"])
    loss_value = np.array(metrics["loss_value"])
    entropy = np.array(metrics["entropy"])
    
    for update in range(config["NUM_UPDATES"]):
        global_step = (update + 1) * config["NUM_ENVS"] * config["NUM_STEPS"]
        
        # Valid returns for this update
        returns = returned_episode_returns[update]
        dones = returned_episode[update]
        ...
        if len(valid_returns) > 0:
            ...
            scores_log.write(json.dumps({"step": global_step, "episode/score": float(mean_return)}) + "\n")
            print(f"global_step={global_step}, episodic_return={mean_return:.2f}")

        # Metrics.jsonl log
        writer.add_scalar("losses/total", loss_total[update], global_step)
        writer.add_scalar("losses/policy", loss_actor[update], global_step)
        writer.add_scalar("losses/value", loss_value[update], global_step)
        writer.add_scalar("losses/entropy", entropy[update], global_step)
        
        metrics_dict = {
            "step": global_step,
            "train/loss/total": float(loss_total[update]),
            "train/loss/policy": float(loss_actor[update]),
            "train/loss/value": float(loss_value[update]),
            "train/ent/action": float(entropy[update]),
        }
    ...
    print(f"Log parsing completed in {time.time() - start_post:.2f}s")
