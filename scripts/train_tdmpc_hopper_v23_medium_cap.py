...
v23 = v13 + latent=256, hidden=(256,256) — medium capacity model.

Findings from prior experiments:
  - v13 (latent=128): continuous improvement, MPPI=388@4M. Target: 594@4M.
  - v17 (latent=512, no RunningScale): collapses at 750k (Q overestimation).
  - v18 (latent=512, RunningScale): stable but plateaus at MPPI=280 (scale dampens pi grad).
  - v20-v22 (K_UPDATE=256): various plateaus; target network issues with >64 gradient steps.

Hypothesis: latent=256 provides 4x more capacity than v13 without requiring RunningScale.
  Q values grow slower (2x fewer channels than 512) → no collapse without RunningScale.
  More capacity → better world model → higher MPPI ceiling.

Changes vs v13:
  1. LATENT=128 → 256
  2. HIDDEN=(128,128) → (256,256)
  3. All other settings unchanged: K_UPDATE=64, no RunningScale, H=3, NS=512
"""
import os, sys, time
import numpy as np
...
def main():
    ...
    csv = f"{out_dir}/hopper-hop-v23.csv"
    ...
    print("Training v23: latent=256, hidden=(256,256) — medium capacity...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
