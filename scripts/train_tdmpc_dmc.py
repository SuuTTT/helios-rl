"""Train TD-MPC2 on dm_control cartpole-balance."""
import os, sys, time
import numpy as np
...
class Encoder(nn.Module):
    latent_dim:int; hidden:tuple=(256,256)
    @nn.compact
    def __call__(self, obs): return NormMLP(self.hidden,self.latent_dim)(obs)

class Dynamics(nn.Module):
    latent_dim:int; hidden:tuple=(256,256)
    @nn.compact
    def __call__(self, z, a):
        return NormMLP(self.hidden,self.latent_dim)(jnp.concatenate([z,a],-1))
...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx, gamma=0.99, rho=0.5, tau=0.01):
    def loss_fn(params, tp, obs_b, act_b, rew_b, done_b):
        ...
        def step_loss(carry, inp):
            ...
            # Stop-gradient on Q params: policy loss only updates pi, not Q
    ...
    def step(params, tp, opt, ob, ab, rb, db):
        ...
        # EMA target update inside JIT to avoid async dispatch flood
...
def make_mppi_fn(enc, dyn, rew_net, q_net, pi_net,
                 ...
                 act_low=-1.0, act_high=1.0, act_dim=1, gamma=0.99):
    ...
    def plan(params, obs, mu, key):
        ...
        def one_iter(carry,_):
            ...
            def rollout_one(args):
                z_i,a_seq=args
                def env_step(z,a):
                    # reward head predicts real reward (linear Q scale)
                ...
                # Q is in discounted-symlog-reward space; clip negatives
...
def eval_mppi(eval_env, plan, params, n_eps, H, act_dim, key):
    ...
    for _ in range(n_eps):
        ...
        mu_e=jnp.zeros((H,act_dim)); er=0.0
        while not te.last():
            act_e,mu_e=plan(params,jnp.asarray(oe),mu_e,ek)
            key,ek=jax.random.split(key)
            te=eval_env.step(np.array(act_e))
...
def main():
    ...
    BS          = 256
    SEQ         = 6
    PI_EVAL_EVERY   = 10_000
    MPPI_EVAL_EVERY = 50_000
    EVAL_EPS    = 5
    LR          = 3e-4
    LATENT      = 128
    HIDDEN      = (256, 256)
    ...
    env     =suite.load("cartpole","balance",task_kwargs={"random":SEED})
    eval_env=suite.load("cartpole","balance",task_kwargs={"random":SEED+1})
    obs_dim =sum(np.prod(v.shape) for v in env.observation_spec().values())
    act_dim =env.action_spec().shape[0]
    al=float(env.action_spec().minimum[0]); ah=float(env.action_spec().maximum[0])
    ...
    # Separate gradient clipping for Q: tighter clip prevents Q divergence
    # from corrupting world model gradients via shared global norm.
    ...
    buf=FastBuffer(500_000,obs_dim,act_dim,SEQ)
    rng=np.random.default_rng(SEED)

    out_dir="/workspace/helios-rl/exp/tdmpc_dmc"; os.makedirs(out_dir,exist_ok=True)
    csv=f"{out_dir}/cartpole-balance.csv"
    with open(csv,"w") as f: f.write("step,reward,seed\n")

    print("Warmup...",flush=True)
    ts=env.reset(); obs=extract_obs(ts)
    for _ in range(WARMUP):
        a=np.random.uniform(al,ah,(act_dim,)).astype(np.float32)
        ts2=env.step(a); buf.add(obs,a,float(ts2.reward),ts2.last())
    ...
    act_e,_=plan(params,jnp.asarray(oe),mu_e,ek); jax.block_until_ready(act_e)
    ...
    while gs<TOTAL:
        ...
        if gs%2_000==0 and gs%PI_EVAL_EVERY!=0:
            elapsed=time.time()-t0
            print(f"  gs={gs:>8,}  sps={gs/elapsed:.0f}  loss={float(loss):.4f}",flush=True)

        if gs%PI_EVAL_EVERY==0:
            pi_ret=eval_pi(eval_env,act_fn,params,EVAL_EPS)
            elapsed=time.time()-t0
            print(f"step={gs:>8,}  pi={pi_ret:6.1f}  sps={gs/elapsed:.0f}  loss={float(loss):.4f}  "
                  f"c={float(aux['c']):.3f} r={float(aux['r']):.3f} v={float(aux['v']):.3f} p={float(aux['p']):.3f}",flush=True)

        MPPI_EVAL_EVERY=20_000
        if gs%MPPI_EVAL_EVERY==0:
            mr,key=eval_mppi(eval_env,plan,params,EVAL_EPS,H,act_dim,key)
            elapsed=time.time()-t0
            print(f"step={gs:>8,}  MPPI={mr:6.1f}  sps={gs/elapsed:.0f}",flush=True)
            with open(csv,"a") as f: f.write(f"{gs},{mr:.1f},{SEED}\n")

    print(f"Done in {time.time()-t0:.0f}s  ->  {csv}")
