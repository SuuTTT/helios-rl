...
v17 = v13 + larger model capacity (latent=512, hidden=(512,512)).

Key finding: official loss coefs require 1:1 gradient ratio+num_q=5+RunningScale to be stable.
v13 stable loss coefs (c=2, r=2, v=1) preserved. Only model capacity increased.
Expected: MPPI@4M > 388 (v13 baseline) due to better dynamics/Q with more capacity.
...
def main():
    ...
    csv = f"{out_dir}/hopper-hop-v17.csv"
    ...
    print("Training v17: latent=512 + v13 stable loss coefs...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
