...
v19 = v18 (latent=512 + RunningScale) + stochastic pi + entropy_coef=1e-4.

Key finding from v18: RunningScale (scale≈9) prevents collapse but dampens pi gradient growth,
causing plateau at MPPI≈280 since 1.25M steps.

Fix: Add entropy bonus to pi loss: pl = -w * mean(min(Q/scale) + entropy_coef * entropy)
The entropy gradient is scale-independent, providing additional gradient signal when Q/scale saturates.
This combination: RunningScale (stability) + entropy (exploration) matches official TD-MPC2.

Changes vs v18:
  1. Stochastic pi: a = tanh(mean + eps * exp(log_std)) with tanh log-prob correction
  2. Entropy term in pi loss: +entropy_coef * (-log_prob) per action sample
  3. Entropy coef = 1e-4 (same as official tdmpc2/config.yaml)
  4. TD target next-state action: stochastic sample from target pi (not deterministic)

All v18 settings preserved:
  - latent=512, hidden=(512,512), K_UPDATE=64
  - RunningScale: IQR of Q_pi, EMA tau=0.01, min=1.0
  - v13 loss coefs: consistency=2, reward=2, value=1, pi=0.1
  - Phase 1 MPPI parity: H=3, NS=512, elites=64, pi_trajs=24
"""
import os, sys, time
import numpy as np
...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx,
                   gamma=0.99, rho=0.5, tau=0.01, rew_scale=10.0, entropy_coef=1e-4):
    def loss_fn(params, tp, obs_b, act_b, rew_b, done_b, rng, scale):
        ...
        def step_loss(carry, inp):
            k, s = carry  # s = running scale (float scalar)
            ...
            # TD target: stochastic pi for next-state value (Phase 2 style)
            pi_mean_n, pi_log_std_n = pi_net.apply(tp["pi"], z_n)
            eps_n = jax.random.normal(sk1, pi_mean_n.shape)
            pi_pre_n = pi_mean_n + eps_n * jnp.exp(pi_log_std_n)
            pi_a_n, _ = squash_logprob(pi_pre_n, gaussian_logprob(eps_n, pi_log_std_n))

            q_next_logits = q_net.apply(tp["q"], z_n, jax.lax.stop_gradient(pi_a_n))
            ...
            # Pi loss: stochastic action + entropy + RunningScale normalization
            ...
            # RunningScale: update IQR from Q_pi values, normalize for pi loss
            q_flat = q_pi2_vals.flatten()
            p5  = jnp.percentile(q_flat, 5)
            p95 = jnp.percentile(q_flat, 95)
            new_s = (1 - tau) * s + tau * jnp.maximum(p95 - p5, 1.0)
            q_pi2_scaled = q_pi2_vals / new_s
            pl   = -w * jnp.mean(jnp.min(q_pi2_scaled, -1) + entropy_coef * entropy)
        ...
        return (2 * jnp.sum(cls) + 2 * jnp.sum(rls) + jnp.sum(vls) + 0.1 * jnp.sum(pls)) / n, \
               {"c": jnp.sum(cls)/n, "r": jnp.sum(rls)/n,
                "v": jnp.sum(vls)/n, "p": jnp.sum(pls)/n, "scale": final_scale}

    @jax.jit
    def single_step(params, tp, opt, ob, ab, rb, db, rng, scale):
        val_and_grad = jax.value_and_grad(loss_fn, has_aux=True)
        (loss, aux), grads = val_and_grad(params, tp, ob, ab, rb, db, rng, scale)
    ...
    def multi_step(params, tp, opt, all_obs, all_acts, all_rews, all_done, key, scale):
        """K gradient updates in one JIT dispatch via jax.lax.scan. Scale is carried."""
        def one_step(carry, batch):
            params, tp, opt, key, scale = carry
            ob, ab, rb, db = batch
            key, uk = jax.random.split(key)
            val_and_grad = jax.value_and_grad(loss_fn, has_aux=True)
            (loss, aux), grads = val_and_grad(params, tp, ob, ab, rb, db, uk, scale)
            ...
            new_scale = aux["scale"]  # updated scale from this gradient step
            return (new_params, new_tp, nopt, key, new_scale), (loss, aux)

        batches = (all_obs, all_acts, all_rews, all_done)
        (params, tp, opt, key, scale), (losses, auxs) = jax.lax.scan(
            one_step, (params, tp, opt, key, scale), batches)
...
def main():
    ...
    REW_SCALE       = 10.0
    ENTROPY_COEF    = 1e-4    # official value from config.yaml
    EXPL_NOISE      = 0.3
    EXPL_UNTIL      = 25_000
    ...
    print(f"v19: latent={LATENT}  hidden={HIDDEN}  K_UPDATE={K_UPDATE}  entropy_coef={ENTROPY_COEF}")
    print(f"     RunningScale + stochastic pi + entropy")
    ...
    single_upd, multi_upd = make_update_fn(enc, dyn, rn, qn, pn, tx, GAMMA, tau=TAU, rew_scale=REW_SCALE, entropy_coef=ENTROPY_COEF)
    ...
    csv = f"{out_dir}/hopper-hop-v19.csv"
    ...
    key, uk = jax.random.split(key)
    params, tp, opt, loss, _ = single_upd(params, tp, opt, ob, ab, rb, db, uk, scale=jnp.float32(1.0))
    jax.block_until_ready(params["enc"])
    print(f"single_upd compiled in {time.time()-t_c:.1f}s  loss={float(loss):.4f}", flush=True)

    scale = jnp.float32(1.0)  # RunningScale initial value
    ...
    params, tp, opt, key, scale, loss, aux = multi_upd(params, tp, opt, ob_k, ab_k, rb_k, db_k, key, scale)
    jax.block_until_ready(params["enc"])
    ...
    print("Training v19: latent=512 + RunningScale + entropy_coef=1e-4...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
    ...
    while env_steps < TOTAL_ENV:
        ...
        if samp_k is not None:
            ob_k, ab_k, rb_k, db_k = [jnp.asarray(x) for x in samp_k]
            params, tp, opt, key, scale, loss, aux = multi_upd(
                params, tp, opt, ob_k, ab_k, rb_k, db_k, key, scale)

        # ---- logging ----
        if env_steps % log_every < N_ENVS:
            elapsed = time.time() - t0_wall
            print(f"  es={env_steps:>11,}  sps={env_steps/elapsed:.0f}  loss={float(loss):.4f}  scale={float(aux['scale']):.2f}", flush=True)
