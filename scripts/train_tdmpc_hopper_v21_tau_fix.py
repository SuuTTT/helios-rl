...
v21 = v20 (K_UPDATE=256) + TAU_PER_STEP = TAU / K_UPDATE (target network fix).

Root cause of v20 plateau at MPPI~234:
  With TAU=0.01 per gradient step and K_UPDATE=256 steps per env step batch:
  After each env step: target = (1-0.01)^256 * old + 0.923 * online
  = 7.7% old + 92.3% online   ← essentially tracking online Q instantly!
  This destroys target network stability (needed for stable TD bootstrapping).
  Official TD-MPC2: 1 gradient step per env step = 99% old + 1% online. ✓

Fix: TAU_PER_STEP = TAU / K_UPDATE = 0.01/256 ≈ 3.9e-5
  After K_UPDATE=256 steps: (1-3.9e-5)^256 ≈ 0.99 = 99% old + 1% online ✓
  This matches the official 1-step target update rate regardless of K_UPDATE.

Key changes vs v20:
  1. TAU_PER_STEP = TAU / K_UPDATE (so effective per-env-step EMA = TAU = 0.01)
  2. K_UPDATE=256 is preserved (4x more gradient steps per env step)
  3. All other settings unchanged

Expected: K_UPDATE=256's fast learning speed + stable target network = better MPPI
"""
import os, sys, time
import numpy as np
...
def main():
    ...
    GAMMA           = 0.99
    TAU             = 0.01
    TAU_PER_STEP    = TAU / K_UPDATE   # target EMA per gradient step; total = TAU per env step
    ...
    single_upd, multi_upd = make_update_fn(enc, dyn, rn, qn, pn, tx, GAMMA, tau=TAU_PER_STEP, rew_scale=REW_SCALE)
    ...
    csv = f"{out_dir}/hopper-hop-v21.csv"
    ...
    print("Training v21: K_UPDATE=256 + TAU_PER_STEP fix (stable target network)...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
