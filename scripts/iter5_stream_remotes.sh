...
while true; do
  ...
  best1=$(awk -F, 'NR>1 && $3=="mppi" {if($2+0>m)m=$2+0} END{printf "%.1f", m}' \
          $MIRROR/ssh3_3060ti/HopperHop_phasey_3060ti/seed_1.csv 2>/dev/null)
  best2=$(awk -F, 'NR>1 && $3=="mppi" {if($2+0>m)m=$2+0} END{printf "%.1f", m}' \
          $MIRROR/ssh3_3060ti/HopperHop_phasey_3060ti/seed_2.csv 2>/dev/null)
  best3=$(awk -F, 'NR>1 && $3=="mppi" {if($2+0>m)m=$2+0} END{printf "%.1f", m}' \
          $MIRROR/ssh3_3060ti/HopperHop_phasey_3060ti/seed_3.csv 2>/dev/null)
  echo "[$ts][stream] ssh3_3060ti phasey s1=${best1:-—} s2=${best2:-—} s3=${best3:-—}"
  ...
  rsync -a -e "ssh -p 11115 -o StrictHostKeyChecking=no -o ConnectTimeout=10" \
        ...
        --include='HopperHop_phasex_4060/' --include='HopperHop_phasex_4060/**/*.csv' --include='HopperHop_phasex_4060/**' \
        --exclude='**/checkpoints/**' --exclude='**/checkpoints' \
