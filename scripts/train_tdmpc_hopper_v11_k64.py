...
v11 = v10 with K_UPDATE=64 (UTD=0.25) — 3.6x faster than K=256.
Sweep showed K=64 reaches 189 reward at 500k steps vs ~95 SPS.
Full 4M step run to validate at scale.

  - N_ENVS=256, K_UPDATE=64, UTD=0.25
  - HIDDEN=(128,128), LATENT=128
  - TOTAL_ENV=4M  → ~3.2 hours wall time at 345 SPS
  - PI eval every 250k, MPPI eval every 500k
"""
import os, sys, time
import numpy as np
...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx,
    ...
    def loss_fn(params, tp, obs_b, act_b, rew_b, done_b, rng, scale_val):
        ...
        def step_loss(carry, inp):
            ...
            # No scale_val division: keeps policy gradient at same magnitude as v5.
            # No entropy term: log_std initialized at -4, fixed (isolates the scale_val fix).
            ...
            q_pi2_vals = two_hot_inv(q_pi2_logits)         # NO / scale_val — keeps gradient at v5 scale
            pl   = -w * jnp.mean(jnp.min(q_pi2_vals, -1)) # no entropy term
...
def make_mppi_fn(enc, dyn, rew_net, q_net, pi_net,
    ...
    def plan(params, obs, mu, key):
        ...
        def pi_step(z, _):
            ...
            a = jnp.tanh(mean_a[0])   # squash to [-1,1] for dynamics consistency
        ...
        def one_iter(carry, _):
            ...
            def rollout_one(args):
                ...
                pi_a_squashed = jnp.tanh(pi_a_mean)  # squash for Q consistency
...
def main():
    ...
    BS              = 256
    SEQ             = 6
    K_UPDATE        = 64          # gradient steps per global step; UTD=64/256=0.25
    LR              = 3e-4
    LATENT          = 128
    ...
    PI_EVAL_EVERY   = 250_000     # env_steps between pi evals
    MPPI_EVAL_EVERY = 500_000     # env_steps between MPPI evals
    EVAL_EPS        = 5
    ...
    csv = f"{out_dir}/hopper-hop-twohot-v11.csv"
