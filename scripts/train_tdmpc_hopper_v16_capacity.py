...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx,
    ...
    def loss_fn(params, tp, obs_b, act_b, rew_b, done_b, rng, scale_val):
        ...
        n = T - 1
        # Official loss coefs: consistency=20, reward=0.1, value=0.1 (Phase 3+4)
        return (20.0 * jnp.sum(cls) + 0.1 * jnp.sum(rls) + 0.1 * jnp.sum(vls) + 0.1 * jnp.sum(pls)) / n, \
...
def main():
    ...
    H               = 3           # Phase 1: was 5
    SEQ             = H + 1       # T = H + 1
    K_UPDATE        = 32   # halved for larger model (maintain reasonable SPS)
    LR              = 3e-4
    LATENT          = 512
    HIDDEN          = (512, 512)
    GAMMA           = 0.99
    ...
    csv = f"{out_dir}/hopper-hop-v16.csv"
