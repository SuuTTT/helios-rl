...
v14 = v13 + Phase 2: Actor/Target Semantics.

Changes vs v13:
  1. Stochastic actor loss: -Q(z, a_sample) - entropy_coef * entropy
     (a_sample = tanh(mean + eps * exp(log_std)) with tanh log-prob correction)
  2. Target Q only: EMA only tp["q"]; use current params for enc/dyn/rew
     (more faithful to official tdmpc2.py which only EMAs the critic target)
  3. Separate optimizer: world_model (enc+dyn+rew+q) uses AdamW, pi uses Adam
     (prevents pi gradient noise from destabilizing world model)
  4. No RunningScale yet (Phase 0 showed Q calibration is decent at ~80% true value;
     adding RunningScale risks overcorrection at low training steps)

All Phase 1 changes preserved:
  - H=3, NS=512, NUM_ELITES=64, NUM_PI_TRAJS=24, adaptive std [0.05,2.0]
  - Elite-based MPPI (top-k), t0 reset

Phase 0 key findings informing this:
  - log_std ≈ -4.0 throughout v13: policy nearly deterministic → entropy bonus critical
  - mppi_mean_ret ≈ expected discounted return: world model well-calibrated
  - Q underestimates true value by ~20%: minor, no RunningScale needed yet
"""
import os, sys, time
import numpy as np
...
def main():
    ...
    csv = f"{out_dir}/hopper-hop-v14.csv"
    ...
    # ── Training loop ────────────────────────────────────────────────────────
    print("Training (Phase 2: stochastic pi + target-Q-only + separate opts)...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
