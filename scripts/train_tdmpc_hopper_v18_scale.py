...
v18 = v17 + RunningScale (JAX port of official tdmpc2 common/scale.py).

Key finding: latent=512 causes Q-overestimation instability without scale normalization.
RunningScale tracks IQR (5th-95th percentile) of Q values, normalizes them for pi loss.
This prevents Q overestimation from destabilizing the policy.

Changes vs v17:
  1. RunningScale: scale = EMA(IQR(Q_pi_vals), tau=0.01), clamped to min=1.0
  2. Pi loss uses scaled Q: pl = -w * mean(min(Q/scale, -1))
  3. scale threaded through loss_fn and multi_step as JAX state (no Python mutation)
  4. scale initialized to 1.0, updated per gradient step within lax.scan

All v17 settings preserved:
...
def main():
    ...
    csv = f"{out_dir}/hopper-hop-v18.csv"
    ...
    print("Training v18: latent=512 + RunningScale...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
