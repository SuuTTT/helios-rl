...
v24 = v18 (latent=512 + RunningScale) + scale_cap=4.0 (prevent over-normalization).

Root cause of v18 plateau at MPPI~280:
  RunningScale grows to scale≈9, dividing pi gradient by 9x.
  Q values grow proportionally with scale → Q/scale stays CONSTANT → pi gradient
  stays constant → no virtuous cycle (better policy → higher Q → stronger gradient).

Fix: Cap scale at 4.0 (scale_min=1.0, scale_max=4.0).
  With scale capped at 4, if Q range grows beyond 4, the normalization STOPS.
  This allows the pi gradient (Q/scale ≈ Q/4) to GROW as Q grows beyond 4.
  Maintains stability (scale bound prevents extreme gradients) + allows growth.

Changes vs v18:
  1. new_s = jnp.clip((1-tau)*s + tau*IQR, 1.0, 4.0)  [was: jnp.maximum(..., 1.0)]
  2. CSV path: hopper-hop-v24.csv

All v18 settings preserved:
  - latent=512, hidden=(512,512), K_UPDATE=64
  - v13 stable loss coefs: consistency=2, reward=2, value=1, pi=0.1
...
def make_update_fn(enc, dyn, rew_net, q_net, pi_net, tx,
    ...
    def loss_fn(params, tp, obs_b, act_b, rew_b, done_b, rng, scale):
        ...
        def step_loss(carry, inp):
            ...
            new_s = jnp.clip((1 - tau) * s + tau * jnp.maximum(p95 - p5, 1.0), 1.0, 4.0)
            q_pi2_scaled = q_pi2_vals / new_s
            pl   = -w * jnp.mean(jnp.min(q_pi2_scaled, -1))
...
def main():
    ...
    print(f"v17: latent={LATENT}  hidden={HIDDEN}  K_UPDATE={K_UPDATE}  (stable v13 coefs + big model)")
    ...
    csv = f"{out_dir}/hopper-hop-v24.csv"
    ...
    print("Training v24: latent=512 + RunningScale capped at 4.0...", flush=True)
    t0_wall = time.time()
    log_every = N_ENVS * 4
