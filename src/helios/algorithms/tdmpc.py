...
class TDMPCAgent(BaseAgent):
    ...
    def act(
        ...
        def _reward(s):
            # Use a zero action placeholder for reward estimation
            dummy_a = jnp.zeros((s["z"].shape[0], state["action_dim"]))
            return rew_mod.apply(params["reward"], s["z"], dummy_a)

        def _value(s):
            dummy_a = jnp.zeros((s["z"].shape[0], state["action_dim"]))
            q_vals = q_fn.apply(state["target_q_params"]["q"], s["z"], dummy_a)
